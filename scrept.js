...
if (grosSalary <= 600) {
  let netIncome = grosSalary;
  taxRate = 0;
  let employeePensiton = 0;
  salaryIncomeTax = 0;
...
} else if (grosSalary >= 601 || grosSalary <= 1650) {
  taxRate = grosSalary * 0.1;
  deduction = 60;
} else if (grosSalary === 1651 || grosSalary <= 3200) {
  taxRate = grosSalary * 0.15;
  deduction = 142.5;
} else if (grosSalary === 3201 || grosSalary <= 5200) {
  taxRate = grosSalary * 0.2;
  deduction = 302.5;
} else if (grosSalary === 5201 || grosSalary <= 7800) {
  taxRate = grosSalary * 0.25;
  deduction = 565;
} else if (grosSalary === 7801 || grosSalary <= 10900) {
  taxRate = grosSalary * 0.3;
  deduction = 955;
} else if (grosSalary >= 10901) {
  taxRate = grosSalary * 0.35;
  deduction = 1500;
  employeePensiton = 0;
  console.log(taxRate, deduction);
}
